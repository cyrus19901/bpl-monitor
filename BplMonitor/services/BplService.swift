...
class BplService: NSObject {
    ...
    static let sharedInstance : BplService = {
        let instance = BplService()
        return instance
    }()
    ...
    public func requestStandyByDelegates(settings: Settings,
                                         listener : RequestListener) -> Void {
        self.requestDelegates(settings: settings, offset: 201, listener: listener)
    }
    ...
    private func requestDelegates(settings: Settings,
        ...
        var urlRequest = BplService.delegatesUrl + "?limit=201&offset=" + String(offset) + "&orderBy=rate:asc"
    ...
    public func requestPeers(settings: Settings,
        ...
        let urlRequest = self.replaceURLWithSettings(url: BplService.activePeersUrl, settings: settings)
    ...
    public func requestAccount(settings: Settings,
        ...
        if (!Utils.validateBplAddress(bplAddress: settings.bplAddress)) {
            let error = NSError(domain: "", code: -1, userInfo: [
                NSLocalizedDescriptionKey: "Invalid Bpl Address"
                ])
        ...
        var urlRequest = self.replaceURLWithSettings(url: BplService.accountUrl, settings: settings)
        urlRequest = urlRequest + "?address=" + settings.bplAddress
    ...
    public func requestVotes(settings: Settings,
        ...
        var urlRequest = self.replaceURLWithSettings(url: BplService.votesUrl, settings: settings)
        urlRequest = urlRequest + "?address=" + settings.bplAddress
    ...
    public func requestVoters(settings: Settings,
        ...
        var urlRequest = self.replaceURLWithSettings(url: BplService.votersUrl, settings: settings)
    ...
    public func requestPeerVersion(settings: Settings,
        ...
        let urlRequest = self.replaceURLWithSettings(url: BplService.peerVersionUrl, settings: settings)
    ...
    public func requestStatus(settings: Settings,
        ...
        let urlRequest = self.replaceURLWithSettings(url: BplService.statusUrl, settings: settings)
    ...
    public func requestForging(settings: Settings,
        ...
        var urlRequest = self.replaceURLWithSettings(url: BplService.forgingUrl, settings: settings)
    ...
    public func requestDelegate(settings: Settings,
        ...
        var urlRequest = self.replaceURLWithSettings(url: BplService.delegateUrl, settings: settings)
    ...
    public func requestLastBlockForged(settings: Settings,
        ...
        var urlRequest = self.replaceURLWithSettings(url: BplService.blocksUrl, settings: settings)
    ...
    public func requestLatestTransactions(settings: Settings,
        ...
        var urlRequest = self.replaceURLWithSettings(url: BplService.transactionsUrl, settings: settings)
        urlRequest = urlRequest + "?senderId=" + settings.bplAddress +
            "&recipientId=" + settings.bplAddress +
    ...
    func replaceURLWithSettings(url: String, settings: Settings) -> String {
        if (!settings.isCustomServer()) {
            let apiUrl : String = url.replacingOccurrences(of: BplService.customApiUrl, with: "")
            return settings.serverType.apiUrl + apiUrl
        }
        
        var apiUrl = url.replacingOccurrences(of:BplService.ipAttr,
                                              with:settings.ipAddress)
        
        apiUrl = apiUrl.replacingOccurrences(of:BplService.portAttr,
                                             with: String(settings.port))
        return (settings.sslEnabled ?
            BplService.httpsProtocol : BplService.httpProtocol) + apiUrl
    }
}
